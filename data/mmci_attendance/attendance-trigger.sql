...
FOR EACH ROW BEGIN
    INSERT INTO timeclock (keylog, log_type, clockin) SELECT * FROM (SELECT (SELECT keylog from employee_time_log WHERE employee_id = NEW.employee_id and log_date = NEW.log_date) as keylogid, 'timeclock', CURTIME()) AS tmp WHERE NOT EXISTS (SELECT keylog FROM timeclock WHERE keylog = keylogid and log_type = 'timeclock' and update_date = '0000-00-00 00:00:00') LIMIT 1;
...
DROP TRIGGER IF EXISTS `after_insert_tmp_ob_log`;

DELIMITER $$
CREATE TRIGGER `after_insert_tmp_ob_log` AFTER INSERT ON timeclock
FOR EACH ROW BEGIN    
...
DROP TRIGGER IF EXISTS `after_update_tmp_ob_log`;

DELIMITER $$
CREATE TRIGGER `after_update_tmp_ob_log` AFTER UPDATE ON timeclock
FOR EACH ROW BEGIN
